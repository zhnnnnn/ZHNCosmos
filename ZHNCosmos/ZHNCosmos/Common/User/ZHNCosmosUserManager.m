...
+ (void)zhn_addUserWithUserName:(NSString *)name password:(NSString *)password success:(ZHNLoginSuccessHandle)success failure:(ZHNLoginFailureHandle)failure {
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:@"https://passport.weibo.cn/signin/login?client_id=787605743&redirect_uri=https://jidian.im/weibo/callback/&display=sdk&action=login&offcialMobile=true&rf=1&version=003143000&sso_type=1"]];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    NSString *prefix = @"https://jidian.im/weibo/callback/";
